...
- **Отчёты** — кнопка в шапке открывает список отчётов для выбранной базы:
  ...
  «Сервис», «Анализ»), «Качество данных» и «Конфигурация» (параметры
  `pg_settings`). Отчёт открывается в модальном окне с экспортом в Excel
  (каждая секция — отдельный лист), HTML и Markdown.
