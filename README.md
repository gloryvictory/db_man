...
- **Обслуживание** — отдельный вид (кнопка в шапке) с заданиями по расписанию:
  VACUUM / ANALYZE / REINDEX на уровне базы данных по простому расписанию
  (ежедневно в ЧЧ:ММ, еженедельно в день недели, каждые N часов). Подсистема
  планировщика следит за расписанием (тик каждые 30 с, догон пропущенных запусков,
  защита от параллельного выполнения) и выполняет задание в назначенное время.
  Для автозапуска у подключения должен быть сохранён пароль. Вкладки «Задания»
  (список + создание/правка/удаление/вкл-выкл/запуск вручную), «История»
  (прогоны: статус, длительность, ошибка) и «Отчёты» (агрегированная статистика
  с экспортом в Excel/CSV).
...
- В «Админке» есть экспорт/импорт данных в JSON: пользователи (с хэшами паролей),
  подключения, сохранённые пароли подключений и задания обслуживания. Файл
  содержит чувствительные данные — храните его надёжно.
