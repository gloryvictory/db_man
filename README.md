...
- **Отчёты** — кнопка в шапке открывает список отчётов для выбранной базы:
  «Общая информация» (собирает вкладки «Информация», «Сервис», «Анализ»),
  «Качество данных» (вкладка «Качество данных») и «Конфигурация» (параметры
  сервера `pg_settings`). Отчёт открывается в модальном окне с экспортом
  в Excel (каждая секция — отдельный лист), HTML и Markdown.
