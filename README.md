...
- **Дерево БД** — иерархия подключений, баз, схем и таблиц с ленивой загрузкой
  ...
  `pg_settings` с поиском) и «DDL» (скрипт создания БД в порядке: база → расширения →
  схемы → таблицы → комментарии → индексы → внешние ключи → представления), клик по
  схеме — «Информация», «Сервис»
  (VACUUM/ANALYZE/REINDEX SCHEMA + агрегированная статистика) и «Анализ».
...
- **Данные** — табличная сетка на TanStack Table: серверная сортировка, фильтр
  ...
  (INSERT-скрипты для переноса данных между окружениями).
...
- **Журнал** — две вкладки: «Запросы» (все выполненные запросы с сервером,
  ...
  индекса — с пользователем, объектом и результатом). Обе — с очисткой.
- **Поиск объектов** — строка в шапке ищет таблицы, колонки и индексы по всем
  схемам текущей БД, клик по результату открывает таблицу.
