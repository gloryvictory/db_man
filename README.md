...
- **Дерево БД** — иерархия подключений, баз, схем и таблиц с ленивой загрузкой
  ...
  Клик по базе данных открывает панели «Информация», «Сервис» (в т.ч. табличные
  пространства) и «Анализ» (таблица
  всех таблиц БД, первая колонка — схема), «Колонки» (все колонки всех таблиц/представлений
  с PK/FK/NOT NULL/default/комментарием, поиск и экспорт в Excel/CSV), «Конфигурация» (параметры сервера из
  ...
  «Качество данных», «Сессии», «Медленные запросы» и «SQL», клик по схеме — «Информация», «Сервис»
  (VACUUM/ANALYZE/REINDEX SCHEMA + агрегированная статистика), «Анализ», «Колонки», «DDL»
  (скрипт создания схемы) и «Качество данных».
  Правая кнопка по базе данных — «Переименовать» (разрывает подключения через
  `pg_terminate_backend` и выполняет `ALTER DATABASE ... RENAME TO`).
...
- **DDL** — сгенерированный `CREATE TABLE` по `pg_catalog` с подсветкой синтаксиса.
...
- **Журнал** — три вкладки: «Запросы» (все выполненные запросы с сервером,
  портом, БД, пользователем и DSN, экспорт в Excel/CSV), «Действия» (аудит
  операций обслуживания — VACUUM/ANALYZE/REINDEX/создание пространственного
  индекса — с пользователем, объектом и результатом, экспорт в Excel/CSV) и
  «Аудит» (статистика входов пользователей: таблица по пользователям и график
  входов по дням).
  Вкладки «Запросы» и «Действия» — с очисткой.
...
- **Качество данных** — на уровне БД и схемы две проверки: «Таблицы без Пространственного
  индекса»
  ...
  обе — с экспортом.
- **Сессии и блокировки** — активные сессии (`pg_stat_activity`) с состоянием,
  ожиданием, запросом и длительностью; завершение/отмена запроса бэкенда
  (`pg_terminate_backend`/`pg_cancel_backend`, пишется в аудит); блокировки (`pg_locks`).
- **Медленные запросы** — топ запросов по суммарному времени выполнения
  (`pg_stat_statements`); если расширение не установлено — подсказка.
- **SQL-консоль** — произвольный SQL (SELECT и DML) на уровне БД: результаты в
  сетке (первые 1000 строк), счётчик затронутых строк для DML, история запросов
  (`localStorage`), `Ctrl+Enter`, подтверждение для DROP/TRUNCATE.
- **Обзор** — вкладки «По размеру» (крупнейшие таблицы по размеру) и
  «По количеству» (таблицы по числу строк), распухание (мёртвые кортежи),
  сводка и список неиспользуемых/дублирующихся индексов (recharts).
- **Отчёты** — кнопка в шапке открывает список отчётов для выбранной базы:
  «Все в одном» (все отчёты разом), «Общая информация» (вкладки «Информация»,
  «Сервис», «Анализ»), «Качество данных», «Конфигурация» (параметры
  `pg_settings`) и «Схема базы данных» (дерево схем → таблицы). Отчёт
  открывается в модальном окне с экспортом в Excel (каждая секция — отдельный
  лист), HTML и Markdown.
...
- **Клавиатурные сокращения** — `Ctrl/Cmd+K` — фокус в строку поиска объектов,
  `F5` — обновить открытую таблицу.
- **Пользователи и роли** — вход по логину/паролю (сессия в HttpOnly-cookie),
  саморегистрация (ФИО, логин, пароль); подключения привязаны к пользователю:
  каждый видит и использует только свои, администратор — все; журнал запросов
  и аудит фильтруются по текущему пользователю (администратор видит все записи).
  Кнопка пользователя в правом верхнем углу: меню «Админка» (только у
  администратора) и «Выйти».

## Пользователи и доступ

При первом запуске создаётся пользователь-администратор по умолчанию:
**логин `admin`, пароль `admin`** (ФИО «Администратор»). Смените пароль сразу
после входа (Админка → «Изменить»).

- Роли: `admin` (администратор) и `user` (обычный пользователь).
- Саморегистрация создаёт пользователя с ролью `user`.
- Администратор управляет пользователями в «Админке» (создание, смена ФИО/пароля/
  роли, удаление); нельзя удалить последнего администратора.
- Пароли хранятся как scrypt-хэш (соль + хэш), сессии — серверные с TTL 7 дней.
- Все API-маршруты (кроме `/api/health` и `/api/auth/*`) требуют входа.
...
Готовый `Dockerfile` (многоступенчатый, Node 22) собирает сервер и клиент в один
образ; сервер раздаёт и API, и SPA:

```bash
docker compose up -d --build
```

- Приложение: http://localhost:3001 (по умолчанию пользователь `admin` / `admin`).
- SQLite (пользователи, подключения, журнал запросов, аудит) — именованный том
  `dbman-data` (каталог `/app/data` внутри контейнера).
- Сервер слушает `0.0.0.0:3001`; при необходимости переопределите `HOST` / `PORT` /
  `SQLITE_PATH` через `environment` в `docker-compose.yml`.
- PostgreSQL наружу не поднимается — db_man подключается к уже существующим базам
  (host/port из настроек подключения). Для локальной СУБД рядом можно
  раскомментировать блок `postgres` в `docker-compose.yml`.
...
│       ├── config.ts
│       ├── sqlite.ts     # node:sqlite (пользователи, сессии, конфигурации, журнал, аудит)
│       ├── auth.ts       # хэш паролей, cookie-сессия, middleware requireAuth/requireAdmin
│       ├── pools.ts      # пулы pg, пароли в памяти
│       ├── ident.ts      # безопасные идентификаторы
│       ├── db.ts         # каталог, данные, сервис/обслуживание (pg_catalog)
│       └── routes/       # connections, catalog, table, export, maintenance, logs, auth, users
└── client/               # React SPA
    └── src/
        ├── store.ts      # zustand
        ├── api.ts
...
| GET | `/api/audit/export?format=xlsx\|csv` | выгрузка журнала действий |
| POST | `/api/auth/login` | вход (`{login, password}`) → сессионная cookie |
| POST | `/api/auth/register` | саморегистрация (`{fio, login, password}`) |
| POST | `/api/auth/logout` | выход (удаляет сессию) |
| GET | `/api/auth/me` | текущий пользователь |
| GET | `/api/users` | список пользователей (админ) |
| POST | `/api/users` | создать пользователя (`{fio, login, password, role?}`) (админ) |
| PUT | `/api/users/:id` | изменить пользователя (ФИО/пароль/роль) (админ) |
