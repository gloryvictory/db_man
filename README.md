...
- **Отчёты** — кнопка в шапке открывает список отчётов для выбранной базы:
  «Общая информация» (собирает вкладки «Информация», «Сервис», «Анализ») и
  «Качество данных» (вкладка «Качество данных»). Отчёт открывается в модальном
  окне с экспортом в Excel (каждая секция — отдельный лист) и в HTML.
